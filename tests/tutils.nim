...
proc testIntToBytes() =
    ...
    for i in 1..20:
        ...
        doAssert bytes.len == 8, "intToBytes() returns byte sequence of incorrect length"
        var num = 0'u64
        for j in 1..bytes.len:
            let b = bytes[^j]
            num = num or ((uint64)(b) shl (8*(j-1)))
        doAssert num == x, "intToBytes() returns incorrect sequence $1 for number $2" % [$bytes, $x]
...
proc testBytesToInt() =
    ...
    doAssert x == r, "bytesToint() returns incorrect integer $1 for the byte sequence $2" % [$r, $bytes]
...
proc testBase32Decode() =
    ...
    doAssert decoded == str, "Test for base32Decode() failed; acutal=$1, output=$2" % [$str, $decoded]
...
proc testBase32Encode() =
    ...
    doAssert encoded == correctEncoding, "Test for testBase32Encode() failed; correctEncoding = $1 but base32Encode() gives $2" % [$correctEncoding, $encoded]
